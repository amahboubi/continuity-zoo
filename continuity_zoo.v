...
Definition IsTree (P : list B -> Type) :=
  forall l l', P l -> P (l ++ l').

Inductive arborification (T : list B -> Type) : list B -> Type :=
| arb l l' : T l -> arborification T (l ++ l').

Lemma arbor_tree P : IsTree (arborification P).
Proof.
  intros l l' H.
  inversion H ; subst.
...
Lemma arbor_barred P : barred P -> barred (arborification P).
Proof.
  intros H alpha ; specialize (H alpha) as [l [Hpref Hl]].
  exists l ; split ; [assumption |].
  erewrite <- cats0 ; now econstructor.
Qed.  



Lemma BI_arbor :
  (forall P : list B -> Type, IsTree P -> BI_ind P) ->
  forall (P : list B -> Type), BI_ind P.
Proof.
  intros HBI P Hbar.
  suff: forall l,
      hereditary_closure (arborification P) l ->
      {n : nat & hereditary_closure P (take n l) }.
  { intros Hyp.
    unfold inductively_barred.
    suff: hereditary_closure (arborification P) nil.
    { intros Hyphered.
      apply Hyp in Hyphered.
      now destruct Hyphered as [n Hn] ; cbn in *.
    }
    apply HBI ; [now apply arbor_tree |].
    now apply arbor_barred.
  }
  clear Hbar HBI ; intros l Hl.
  induction Hl.
