...
(* Analogues of eqseq_cons for non eqTypes. Caveat: bias in the size equality,
  sometimes the other one would be more direct. *)
...
Lemma eq_catl {T : Type} (s3 s4 s1 s2 : seq T) :
...
(*   Forall_nil : Forall P [::] *)
(* | Forall_cons : forall (x : A) (l : seq A), P x -> Forall P l -> Forall P (x :: l). *)

Section Bars.
Variables A B : Type.
Implicit Type (T : seq (A * B) -> Type).

(* Herbelin-Brede's generalized barred *)
Definition HBbarred T :=
  forall α : A -> B, { u : list (A * B) & (Forall (fun '(a,b) => α a = b) u * T u)%type }.

(* A more handy version for concrete formal proofs *)
Definition barred T :=
  forall α : A -> B, {u : list A & T [seq (i, α i) | i <- u]}.

(* barred and HBbarred coincide *)
Lemma HBbarredP T :
  (HBbarred T -> barred T) * (barred T -> HBbarred T).
Proof.
split=> h sa; case: (h sa) => u hu.
- exists (map fst u).
  case: hu => [uext Tu].
  suff <- : u = [seq (i, sa i) | i <- [seq i.1 | i <- u]] by done.
  elim: u uext {Tu} => [| [a b] u ihu] //= hF.  
  inversion_clear hF as [|h1 h2 h3 h4].  
  by rewrite h3 {1}(ihu h4).
- exists [seq (i, sa i) | i <- u].
  split=> // {hu}.  
  by elim: u => // a u ihu //; constructor.
Qed.

Inductive indbarred T : list (A * B) -> Type :=
  | ieta u' u : T u' -> indbarred T (u' ++ u)
  | ibeta a v : ~ List.In a (map fst v) ->
              (forall b, indbarred T (v ++ [:: (a,b)])) -> indbarred T v.

Inductive indbarred_spec T :  list (A * B) -> Type := 
...
Lemma indbarredP T (b : B) (l : list (A * B)) : indbarred T l -> indbarred_spec T l.
Proof.
...
- destruct v using last_ind.
  ...
  +  pose y := ih b. inversion y. 
     ...
     * have -> : rcons v x = u' ++ u.
         apply: (@eq_catl _ [:: (a, b)] [:: (a0, b0)]); last by rewrite -H0 cats1.
         move: (f_equal size H0).
         by rewrite size_rcons [size (_ ++ [:: _])]size_cat addn1; case.
       destruct u using last_ind.
       - by rewrite cats0; apply: (Eta [::]); rewrite cats0.
       - rewrite -rcons_cat; case: x0 H0 => [aa  bb] H0; exact: Beta.
Qed.

(* indbarredP is just a more convenient phrasing of the previous indbarred_lem. 
  Just for the record let's prove they are equivalent *)
Definition indbarred_lem T (l : list (A * B)) : Type :=
  { u' & { u & { a & {b & ((l = u' ++ u ++ [:: (a,b)]) * T u')%type}}}} + { u & T (l ++ u)}.

Lemma indbarred_lem_indberredP T (l : list (A * B)) :
  (indbarred_spec T l -> indbarred_lem T l) * (indbarred_lem T l -> indbarred_spec T l).
Proof.
rewrite /indbarred_lem; split.
-  case=> [u v Tuv | u v a b Tu]; first by right; exists u.
   left; exists u; exists v; exists a; exists b; split=> //.
   by rewrite rcons_cat cats1.
- case; last first.
  + case=> [u Tlu]; exact: (Eta u).
    case=> u [v [a [b [-> Tu]]]].
    rewrite cats1 -rcons_cat; exact: Beta.
Qed.

(* How to use inbarredP *)
Lemma indbarredP_demo T (b : B) (l : list (A * B)) P : 
   indbarred T l -> P l.
Proof.
 (* here we could dd case/(indbarred_lem b) *)
 (* but let's assume the hypothesis is in the context *)
 move=> h. 
 case/(indbarredP b): h. (* analogue of case (indbarredP b h). *)
Admitted.

(* Generalized Bar Induction, phrased using the Herbelin-Brede way *)
Definition GBI T := HBbarred T -> indbarred T [::].

End Bars.

(**)
Theorem GBI_to {I O A} (F : (I -> O) -> A) {i : I} :
  (forall (T : list (I * O) -> Type), GBI T) ->
  seq_contW F -> @dialogue_cont I O A F.
Proof. Admitted.
